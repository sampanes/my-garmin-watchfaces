...
| 🔵 | **Plays you a tune** | `Attention.playTone` (19 named tones) + custom `ToneProfile` melody (freq+dur sequence) | EXPRESSIVE | 🔵 FR265 only | [OFFICIAL] | A little victory jingle on goal-hit. **VA6 lacks a tone generator** per docs → `playTone` likely no-ops there. ⚠️ FR265 hardware first pass (2026-06-25, SW 28.05, T1) produced **no audible beep** for `TONE_SUCCESS`; retest sound settings + custom `ToneProfile` before relying on tones. |
|   | **Haptic emote / "morse"** | `Attention.vibrate([VibeProfile,…])` (≤8 profiles, duty% + ms) | EXPRESSIVE | ⚪ (rhythm 🟢-leaning) | [OFFICIAL] | Pet "speaks" in buzz patterns (double-tap = hi, long pulse = hungry). ✅ **FR265 = flat single buzz only** confirmed on hardware (2026-06-25, SW 28.05, T2): 25/100/25 profile sequence produced no distinct pattern. VA6/Venu-class pattern support still needs its own T2 run. |
...
| ⭐ | **Encounter (in-person, no backend)** | `Ant.GenericChannel` master `CHANNEL_TYPE_TX_NOT_RX` on `NETWORK_PUBLIC`, 8-byte payload (avoid RF 57) | FUNCTIONAL | ⚪ | [FORUM] ⚠️ | Two pets detect each other over ANT (~3–5 m) → visit/gift/XP. The no-phone social layer (`03 §Spike B`, `02`). Ran clean on a Forerunner 745; "Chess! – Blind Knights" is a shipped watch-to-watch ANT app (its device-support list is still unconfirmed — `03 §Spike B` checklist). ⚠️ VERIFY TX on FR265+VA6 + payload arrival across two watches (T23/T24). **BLE watch-to-watch is impossible** (central-only) — ANT is the only path. |
